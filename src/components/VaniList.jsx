...
export default function VaniList({
  ...
  const applyFilters = (filterId) => {
    ...
    // Remove old values for this filter
  ...
  filterConfigs.forEach((config) => {
    ...
    values.forEach((value) => {
      const item = filters[config.id]?.find((i) => i.value === value);
      if (item) {
        activeTags.push({
          filterId: config.id,
    ...
    <>
              ...
              {isOpen && (
                <div className="fixed sm:absolute top-0 left-0 sm:top-full sm:left-auto right-0 sm:right-auto sm:mt-2 w-full sm:w-72 h-full sm:h-auto bg-white sm:rounded-xl border-0 sm:border border-text-body/10 shadow-2xl sm:shadow-xl z-50 overflow-hidden">
                  ...
                  <button
                    ...
                    className="sm:hidden absolute top-4 right-4 w-8 h-8 flex items-center justify-center rounded-full bg-text-body/5 hover:bg-text-body/10 transition-colors z-10"
                  >
                    <svg
                    ...
                    {filterItems.map((item) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full flex items-center gap-3 px-4 py-2.5 sm:py-2 text-left cursor-pointer transition-colors duration-150 hover:bg-orange-50 active:bg-orange-100 ${
                            isChecked ? "bg-section-bg" : ""
                          }`}
                        >
                          {/* Checkbox */}
                          <span
                            className={`flex-shrink-0 border-2 rounded flex items-center justify-center transition-all duration-150 ${
                              isChecked
                                ? "bg-primary border-primary"
                                : "bg-white border-text-body/25"
                            }`}
                          ...
                          {/* Label */}
                          <span className="flex-1 text-text-body text-sm font-medium">
                            {item.label}
                          </span>

                          {/* Count */}
                          <span className="text-xs font-semibold text-text-body/50 bg-section-bg px-2 py-0.5 rounded-full">
                            {item.count}
                          </span>
                        </button>
                      );
                  ...
                  <div className="flex gap-2 px-3 py-3 border-t border-text-body/10 bg-white">
                    <button
                      ...
                      className="flex-shrink-0 px-4 py-2 sm:py-1.5 border border-text-body/20 rounded-lg text-xs font-semibold text-text-body/60 cursor-pointer transition-all duration-150 hover:border-primary hover:text-primary active:scale-95"
                    >
                      Clear
                    </button>
                    <button
                      onClick={() => {
                        applyFilters(config.id);
                        closeDropdown();
                      }}
                      className="flex-1 px-4 py-2 sm:py-1.5 bg-primary rounded-lg text-xs font-bold text-white cursor-pointer transition-all duration-150 hover:bg-primary-hover hover:shadow-md active:scale-95"
                    >
                      Apply
                    </button>
                  </div>
      ...
      <div className="flex flex-wrap gap-2 mb-3 sm:mb-4 items-center" style={{ minHeight: "28px" }}>
        {activeTags.map((tag, idx) => (
          <span
            key={`${tag.filterId}-${tag.value}-${idx}`}
            className="inline-flex items-center gap-1.5 bg-primary text-white text-xs font-semibold px-2.5 sm:px-3 py-1 rounded-full"
          >
            ...
            <button
              ...
              className="flex items-center justify-center rounded-full bg-white/25 hover:bg-white/40 active:bg-white/50 transition-colors cursor-pointer border-0 p-0"
            ...
            >
              <svg
                ...
                strokeWidth="3"
                viewBox="0 0 24 24"
                style={{ width: "9px", height: "9px", color: "#fff" }}
              >
        ...
        {lectures.length === 0 ? (
          <div className="text-center py-12">
            ...
            <button
              ...
              className="mt-4 inline-flex items-center gap-2 bg-primary text-white px-6 py-2.5 rounded-full text-sm font-semibold shadow-sm cursor-pointer transition-all duration-200 hover:bg-primary-hover hover:shadow-md active:scale-95"
            >
              Clear all filters
            </button>
          </div>
          ...
          <>
              ...
              >
                ...
                <div className="flex flex-col gap-0.5 sm:gap-0.5">
                  <div className="flex items-baseline gap-2 sm:gap-2.5">
                    <span
                      className="text-xs font-bold text-text-body/25 shrink-0"
                      style={{
                        width: "18px",
                        textAlign: "right",
                ...
                {/* Right: chevron */}
                <svg
                  className="hidden sm:block w-5 h-5 text-text-body/20 group-hover:text-primary transition-all duration-200 group-hover:scale-110 shrink-0 ml-2"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2"
                  viewBox="0 0 24 24"
            ...
            <div className="mt-8 sm:mt-10 lg:mt-12 flex items-center justify-center gap-2 sm:gap-3">
              ...
              {currentPage > 1 ? (
                ...
                <span className="inline-flex items-center gap-1.5 sm:gap-2 bg-white border border-text-body/20 text-text-body/40 px-4 sm:px-5 lg:px-6 py-2 sm:py-2.5 rounded-full text-xs sm:text-sm font-semibold shadow-sm cursor-not-allowed">
                  <svg
                    className="w-3.5 h-3.5 sm:w-4 sm:h-4"
                    fill="none"
                    stroke="currentColor"
              ...
              {currentPage < totalPages ? (
                <button
                  onClick={() => changePage(currentPage + 1)}
                  className="inline-flex items-center gap-1.5 sm:gap-2 bg-primary text-white px-4 sm:px-5 lg:px-6 py-2 sm:py-2.5 rounded-full text-xs sm:text-sm font-semibold shadow-sm cursor-pointer transition-all duration-200 hover:bg-primary-hover hover:shadow-lg active:scale-95"
                >
                ...
                <span className="inline-flex items-center gap-1.5 sm:gap-2 bg-primary/40 text-white px-4 sm:px-5 lg:px-6 py-2 sm:py-2.5 rounded-full text-xs sm:text-sm font-semibold shadow-sm cursor-not-allowed">
                  <span className="hidden xs:inline">Next</span>
                  <svg
                    className="w-3.5 h-3.5 sm:w-4 sm:h-4"
                    fill="none"
